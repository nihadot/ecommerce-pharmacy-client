...
const Viewbannertwo = () => {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await axios.delete(
        `http://localhost:3000/api/bannertwo/${id}`
      );
      setRefresh(!refresh);
      successToast(response.data.message);
      navigate("/admin/viewbannertwo");
    } catch (error) {
  ...
  return (
    <div className="ms-[30%] mt-[5%] font-sans">
      <Link to={"/admin/addbannertwo"} className="hover:underline">
        <span className="text-[25px] text-emerald-950"> Add Banner 2</span>
      </Link>
                ...
                return (
                  <>
                    ...
                    >
                      ...
                      >
                        {" "}
                        <button className=" text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg text-md w-full sm:w-auto px-5 py-2.5 text-center dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800">
                          Delete
                        </button>
                      </td>
                    </tr>
