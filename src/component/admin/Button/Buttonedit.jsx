...
function Buttonedit() {
    ...
    const editCards = async(e) =>{
        e.preventDefault()
        try {
            const response = await axios.put(`http://localhost:3000/api/buttons/${id}`,{image:image,title:title,description:description,cost:cost,timeline:timeline,offerRate:offerRate},
            {
                headers:{
