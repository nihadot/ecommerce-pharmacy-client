...
function Addadver2() {
    ...
    const addBanner = async(e) =>{
        e.preventDefault()
        try {
            const response = await axios.post(`http://localhost:3000/api/banner2`,{image:image,title:title,percentage:percentage,rate:rate,offerRate:offerRate},{headers:{
        ...
        <form action="" onSubmit={addBanner}>
            ...
            <div className="flex flex-col w-[200px] gap-3 m-auto mt-3">
                <div className="flex flex-col gap-3">
                    ...
                    <input type="text" placeholder='rate' name='rate' value={rate} onChange={(e)=>setRate(e.target.value)} className='outline outline-1 text-xs sm:text-base  px-2' />
