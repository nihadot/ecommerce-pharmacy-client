  ...
  const fetchAddress = async () => {
    try {
      const response = await axios.get(
        `http://localhost:3000/api/geetorders/getById/${JSON.parse(localStorage.getItem("userDetails"))?._id}`
      );
      setAddress(response.data.data);

      const response___ = await axios.get(
        `http://localhost:3000/api/user/getById/${JSON.parse(localStorage.getItem("userDetails"))?._id}`
      );
...
<div className="w-[640px] flex px-[10%] py-[5%] gap-[40px] font-sans bg-blue-200">
              ...
              <h1 className="text-white font-bold ">Your order items</h1>
                        <p>totals amt:{item.total}</p>

                      <div className="flex flex-col pt-[10px]">
                        { item?.productsArray && item?.productsArray.map((i)=>{
                        {console.log(i,'--')}
                          return(
                            <Card
...
<Link to={'/order' } state={item}>
                      <p className="border-blue-300">print</p>
</Link>
                    </>
                  );
                })}
